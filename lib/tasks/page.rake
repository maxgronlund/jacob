namespace :page do
  ...
  task build_front_pages: :environment do
    user = User.super_admin
    pages_params = [
      { title: 'Forside', menu_title: 'Jacob', menu_id: 'Ingen', locale: 'da', layout: 'alabama', active: true, user_id: user.id },
      { title: 'Front page', menu_title: 'Jacob', menu_id: 'None', locale: 'en', layout: 'alabama', active: true, user_id: user.id }
    ]
